...
int on_data_payload(void *user_data, const char *buff, size_t len)
{
        ...
        if (strcmp(dataType->field_name, WS_LOGIN) == 0) {
                // login
                ...
                ws_info->client_info->first = malloc(login->field_val->second * sizeof(char));
                strcpy(ws_info->client_info->first, (char *)login->field_val->first);

                bst_insert(ws_info->network_manager->ws_clients, ws_info->client_info);
        } else if (strcmp(dataType->field_name, WS_SEND_MESSAGE)) {
                // Send message
                json_data *reciver = parse_json_row(&request);
                printf("'%s':'%s'\n", (char *)reciver->field_name->first, (char *)reciver->field_val->first); // TODO::DELETE

                struct pair search_data = {reciver->field_val, 0ll};
                ...
                if (node) {
                        ...
                        printf("Login: %s is not connected now", reciver->field_val);
                }
